...
public class Code_Shieldattack : MonoBehaviour
{
    ...
    void Update()
    {
        lenghtcheck = player.gameObject.transform.position.x - this.gameObject.transform.position.x;
        //print(lenghtcheck);
        //print(attacklist);
        if ((lenghtcheck<-4) || (lenghtcheck > 4))
        {
            attacklist = 2;
        }
        else if ((lenghtcheck >= -4) || (lenghtcheck <= 4))
        {
            attacklist = 1;
        }
        if (attackcount == 1)
    ...
    private void playerAttack()
    {
        ...
        if (enemy.gameObject.GetComponent<Code_Shieldbehav>().facingright == true)
        {
            castDist = distance;
        }
        else
