...
public class Code_Swordmanbehav : MonoBehaviour
{
    //-------------------movement-----------------------
    ...
    void Update()
    {
        if (foundplayer && (!confusestate))
        {
            ...
            if (attacker.GetComponent<Code_swordmanatkcheck>().attackcount == 1 && !lunging)
            {
                lunging = true;
    ...
    private IEnumerator lunge()
    {
        ...
        attacker.GetComponent<Code_swordmanatkcheck>().attackcount = 0;
        attack = false;
        lunging = false;
